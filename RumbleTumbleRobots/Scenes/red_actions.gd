...
func _process(delta):
	...
	elif Input.is_action_just_pressed("Red_Dodge"):
		dodge()
...
func npc_loop():
	while true:
		...
		if not npc_mode:
			continue
		if dead or is_punching or is_dodging or action_cd > 0.0:
			continue
		if randi() % 2 == 0:
			punch()
			action_cd = punch_cooldown
...
func die():
	...
	sprite.play("defeat")
	log_event("death")
	match_manager.end_match("Blue", enemy.dodge_cooldown)
	export_csv()
...
func _on_button_pressed() -> void:
	npc_mode = !npc_mode
	log_event("toggle_npc", int(npc_mode))
